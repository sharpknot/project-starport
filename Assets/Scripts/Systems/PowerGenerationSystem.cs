...
namespace Starport.Systems
{
    public class PowerGenerationSystem : SystemBase
    {
        ...
        {
            while (!AreSubsystemsInitialized(PipeSubsystems)) yield return null;
            SubscribePipeEvents();
            OnPipeSubsystemUpdated?.Invoke(PipeSubsystems);

            while (!AreSubsystemsInitialized(ReservoirSubsystems)) yield return null;
            SubscribeReservoirEvents();
            OnReservoirSubsystemsUpdated?.Invoke(ReservoirSubsystems);

            while (!AreSubsystemsInitialized(ReactorSubsystems)) yield return null;
            SubscribeReactorEvents();            
            OnReactorSubsystemsUpdated?.Invoke(ReactorSubsystems);

            while(!AreSubsystemsInitialized(GeneratorSubsystems)) yield return null;
            SubscribeGeneratorEvents();
            OnGeneratorSubsystemsUpdated?.Invoke(GeneratorSubsystems);

            while (!AreSubsystemsInitialized(RegulatorSubsystems)) yield return null;
        ...
        private bool AreSubsystemsInitialized(SubsystemBase[] subsystems)
        {
